...
def get_db_metadata(conn, db_name):
    ...
    with conn.cursor() as cursor:
        ...
        """, (db_name,))
        metadata["triggers"] = [row["TRIGGER_NAME"] for row in cursor.fetchall()]
...
def verify_database(aws_config, azure_config, db_name):
    """
    ...
    try:
        ...
        if aws_tables != az_tables:
            missing_in_az = aws_tables - az_tables
            extra_in_az = az_tables - aws_tables
            if missing_in_az:
        ...
        # Compare other object lists
        for obj_type in ["views", "procedures", "functions", "triggers", "events"]:
            aws_objs = set(aws_meta[obj_type])
            az_objs = set(az_meta[obj_type])
            if aws_objs != az_objs:
                mismatches.append(f"{obj_type.capitalize()} mismatch. Missing: {aws_objs - az_objs}. Extra: {az_objs - aws_objs}")

        # 3. Compare Table Column schemas and Row Counts
        for table in (aws_tables & az_tables):
            aws_tbl = aws_meta["tables"][table]
            az_tbl = az_meta["tables"][table]
            ...
            if aws_row_count != az_row_count:
                mismatches.append(f"Table '{table}' row count mismatch! AWS: {aws_row_count}, Azure: {az_row_count}")
                
            # Engine and collation check
            if aws_tbl["engine"] != az_tbl["engine"]:
                mismatches.append(f"Table '{table}' engine mismatch. AWS: {aws_tbl['engine']}, Azure: {az_tbl['engine']}")
            if aws_tbl["collation"] != az_tbl["collation"]:
                mismatches.append(f"Table '{table}' collation mismatch. AWS: {aws_tbl['collation']}, Azure: {az_tbl['collation']}")
                
            # Columns metadata check
            aws_cols = set(aws_tbl["columns"].keys())
            az_cols = set(az_tbl["columns"].keys())
            if aws_cols != az_cols:
                mismatches.append(f"Table '{table}' columns mismatch. Missing in Azure: {aws_cols - az_cols}. Extra: {az_cols - aws_cols}")
            ...
            for col in (aws_cols & az_cols):
                ...
                if ac["data_type"] != zc["data_type"]:
                    mismatches.append(f"Table '{table}' Column '{col}' data type mismatch: AWS={ac['data_type']}, Azure={zc['data_type']}")
                if ac["nullable"] != zc["nullable"]:
                    mismatches.append(f"Table '{table}' Column '{col}' nullability mismatch: AWS={ac['nullable']}, Azure={zc['nullable']}")
                if ac["default"] != zc["default"]:
                    mismatches.append(f"Table '{table}' Column '{col}' default mismatch: AWS={ac['default']}, Azure={zc['default']}")
                    
            # Primary / Unique Indexes check
            aws_idxs = set(aws_tbl["indexes"].keys())
            az_idxs = set(az_tbl["indexes"].keys())
            if aws_idxs != az_idxs:
                mismatches.append(f"Table '{table}' indexes mismatch. AWS keys: {aws_idxs}. Azure keys: {az_idxs}")
            else:
                for idx in aws_idxs:
                    ai = aws_tbl["indexes"][idx]
                    zi = az_tbl["indexes"][idx]
                    if ai["unique"] != zi["unique"] or ai["columns"] != zi["columns"]:
                        mismatches.append(f"Table '{table}' Index '{idx}' definition mismatch. AWS={ai}, Azure={zi}")
            ...
            if aws_fks != az_fks:
                ...
                if aws_fk_mappings != az_fk_mappings:
                    mismatches.append(f"Table '{table}' Foreign Keys mismatch. AWS mapping: {aws_fk_mappings}, Azure mapping: {az_fk_mappings}")
            ...
            if aws_row_count > 0:
                ...
                # Fetch AWS checksum
                with aws_conn.cursor() as cur:
                    cur.execute(f"{checksum_sql} `{table}`")
                ...
                if aws_checksum != az_checksum:
                    mismatches.append(f"Table '{table}' data checksum mismatch! AWS: {aws_checksum}, Azure: {az_checksum} (Data integrity compromise)")
                    log_error(db_name, table, "Data Checksum", 0, "FAILED", f"AWS Hash={aws_checksum}, Azure Hash={az_checksum}")
                else:
                    log_verification(db_name, table, "Data Checksum Match", 0, f"Checksum = {aws_checksum}")
            else:
                log_verification(db_name, table, "Data Checksum Match", 0, "SUCCESS (Empty Table)")
